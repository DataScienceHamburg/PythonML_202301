...
# %% data prep
X, y = make_multilabel_classification(n_samples=10000, n_features=10, n_classes=3, n_labels=2, random_state=123)
...
model = MultilabelNetwork(input_size=input_dim, hidden_size=5, hidden_size2=5,output_size=output_dim)
model.train()
# %%
...
for epoch in range(number_epochs):
    ...
    losses.append(loss_current_epoch)
    print(f"Epoch {epoch}, Loss: {loss_current_epoch}")
    

# %% losses
sns.lineplot(x=range(len(losses)), y=losses, alpha=0.1)
plt.show()
# %% test the model
